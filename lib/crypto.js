...
import { generateKeyPairSync, sign, verify, diffieHellman,
         ...
         createCipheriv, createDecipheriv, hkdfSync, createHash } from 'node:crypto';
...
export async function encryptForRecipient(plaintext, recipientX25519PubBase64, senderEd25519PrivBase64) {
  ...
  // Hash plaintext for guardrail verification (relay can verify without decrypting)
  const plaintextHash = createHash('sha256').update(plaintext, 'utf8').digest('base64');

  // Sign ciphertext:ephemeralKey:nonce:plaintextHash with sender's Ed25519 key
  // Proves: sender identity + binds plaintext to ciphertext (guardrail can't be abused)
  const senderSig = await signMessage(`${ciphertext}:${ephemeralKey}:${nonceB64}:${plaintextHash}`, senderEd25519PrivBase64);

  return { ciphertext, ephemeralKey, nonce: nonceB64, senderSig, plaintextHash };
}
