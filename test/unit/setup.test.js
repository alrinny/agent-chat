...
describe('setup.sh — 409 handling', () => {
  it('SETUP-409-001: fresh handle registers successfully', () => {
    ...
    const out = execSync(
      `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} 2>&1`,
      { encoding: 'utf8' }
    );
  ...
  it('SETUP-409-002: duplicate handle with matching keys reuses registration', () => {
    ...
    execSync(
      `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${freshHandle} --no-daemon 2>&1`,
      { encoding: 'utf8' }
    );
...
describe('setup.sh — --daemon flag', () => {
  it('SETUP-DAEMON-001: default setup creates LaunchAgent plist on macOS', () => {
    ...
    try {
      ...
      const out = execSync(
        `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_CHAT_ID=123 AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} 2>&1`,
        { encoding: 'utf8' }
      );
  ...
  it('SETUP-DAEMON-002: --no-daemon skips LaunchAgent, shows manual instructions', () => {
    const randomHandle = `test-${Date.now().toString(36)}`;
    const out = execSync(
      `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} --no-daemon 2>&1`,
      { encoding: 'utf8' }
    );
