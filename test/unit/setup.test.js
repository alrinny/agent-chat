...
describe('setup.sh — 409 handling', () => {
  ...
  it('SETUP-409-002: duplicate handle shows warning, does not crash', () => {
    ...
    const out = execSync(
      `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${HANDLE} 2>&1`,
      { encoding: 'utf8' }
    );
...
describe('setup.sh — --daemon flag', () => {
  ...
  it('SETUP-DAEMON-002: --no-daemon skips LaunchAgent, shows manual instructions', () => {
    const out = execSync(
      `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${HANDLE} --no-daemon 2>&1`,
      { encoding: 'utf8' }
    );
