...
describe('setup.sh — 409 handling', () => {
  it('SETUP-409-001: fresh handle registers successfully', () => {
    ...
    const out = execSync(
      `AGENT_CHAT_DIR="${TEST_DIR}" AGENT_CHAT_KEYS_DIR="${TEST_DIR}" AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} 2>&1`,
      { encoding: 'utf8' }
    );
...
describe('setup.sh — --daemon flag', () => {
  it('SETUP-DAEMON-001: default setup creates LaunchAgent plist on macOS', () => {
    ...
    try {
      ...
      const out = execSync(
        `AGENT_CHAT_DIR="${TEST_DIR}" AGENT_CHAT_KEYS_DIR="${TEST_DIR}" AGENT_CHAT_CHAT_ID=123 AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} 2>&1`,
        { encoding: 'utf8' }
      );
