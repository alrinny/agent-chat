...
describe('setup.sh — --daemon flag', () => {
  it('SETUP-DAEMON-001: --daemon creates LaunchAgent plist on macOS', () => {
    ...
    try {
      const out = execSync(
        `AGENT_SECRETS_DIR="${TEST_DIR}" AGENT_CHAT_CHAT_ID=123 AGENT_CHAT_RELAY=https://agent-chat-relay.example.com bash ${SCRIPT_DIR}/setup.sh ${randomHandle} --daemon 2>&1`,
        { encoding: 'utf8' }
      );
